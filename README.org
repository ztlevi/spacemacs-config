...
*** Step3: Configuration and Dependencies
    ...
    4. Dependencies needed
       ...
       - npm
         #+BEGIN_SRC shell
           npm install -g prettier live-server tern import-js javascript-typescript-langserver flow-language-server
         #+END_SRC
       - pip
...
** Usage
   ...
   6. Encrypt org entries by using ~C-c C-c~ to set ~secret~ tag for the entry. Save the file and input the passcode. Then the entry will be encrypted. Decrypt the entry by using ~org-decrypt-entry~.
